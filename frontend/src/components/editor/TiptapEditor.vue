...
/* Default typography. The editor and the DOCX export share these values so
   ...
   src/components/editor/typographyDefaults.ts. ``line-height: 1`` keeps
   each line box exactly font-size tall — the export mirrors this with
   ``line=280 lineRule=exact`` (= 14pt × 20 twips) so both renderers fit
   the same number of lines per page. Anything larger here lets the
   browser inflate lines past Word's exact rendering and the editor ends
   up with noticeably more pages than the exported DOCX. */
.tiptap-editor,
.tiptap-mini {
